...
This project is not meant to replace ETABS, STAAD, or SAP2000. It is a transparent structural-analysis automation tool that demonstrates the core logic behind such software.

A good way to describe it:

> I developed a Python-based RC frame analysis and preliminary design tool. The program generates a 2D building frame from user inputs, assembles the global stiffness matrix, applies gravity and lateral loads, solves for nodal displacements, recovers member forces, creates design envelopes from IS-style load combinations, plots SFD/BMD diagrams, performs preliminary IS-code-aware checks, validates results using benchmark/equilibrium checks, and generates PDF and Excel reports. The main purpose was to demonstrate understanding of structural analysis fundamentals and automate a repetitive engineering workflow.
